...
extern int
do_dhcp_renew(const char *intf, struct bootp *res,
        cyg_uint8 *pstate, struct dhcp_lease *lease);


typedef struct _dhcp_time_out_config
{
    long req_tval;              ///microsecond
    long req_max_cnt;           ///the request count in a cycle
    long req_next_cyc_tval;     ///second
    long req_cyc_max_cnt;       ///max. retry cycle count
}dhcp_time_out_config;

extern int dhcp_set_req_timeout_cnt(dhcp_time_out_config *pdhcptoconf);


enum dhcp_cb_type
{
    DHCP_CB_TYPE_CLI_REQ_CNT = 0 ,          ///< DHCP Client Request Cycle and count
    DHCP_CB_TYPE_CLI_REQ_RESULT ,           ///< DHCP Client Request IP success or fail(0)
    DHCP_CB_TYPE_MAX
};

typedef void (*dhcp_cb_func)(enum dhcp_cb_type cbType, unsigned int p1, unsigned int p2, unsigned int p3);
extern void dhcp_reg_cb(dhcp_cb_func pFunc);
