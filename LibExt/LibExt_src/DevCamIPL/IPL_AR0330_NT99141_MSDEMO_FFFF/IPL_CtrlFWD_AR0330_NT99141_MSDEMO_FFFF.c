...
void IPL_CtrlInitIpcInfoFlowD_CCIR(UINT32 Id, IPC_INFO *IpcInfo)
{
    ...
    if (IpcInfo->ICF_FD.CCIR.CCIR_En == ENABLE)
    {
        ...
        //IPL_CtrlInitIpc_SIE2Signal(Id, &SenInfo, TRUE, TRUE, _FALLING, _RISING, _RISING, &IpcInfo->ICF_FD.CCIR.SenSorIn);
        //IPL_CtrlInitIpc_SIE2Signal(Id, &SenInfo, TRUE, TRUE, _FALLING, _RISING, _FALLING, &IpcInfo->ICF_FD.CCIR.SenSorIn);
        //IPL_CtrlInitIpc_SIE2Signal(Id, &SenInfo, TRUE, TRUE, _RISING, _FALLING, _FALLING, &IpcInfo->ICF_FD.CCIR.SenSorIn);
        #endif
    }
