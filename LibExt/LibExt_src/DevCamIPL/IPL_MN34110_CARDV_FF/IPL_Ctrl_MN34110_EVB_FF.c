    ...
    if (IPL_CtrlGetInfor(Id, IPLCTRL_SIE_ECS_EN) == ENABLE)
    {
        ...
        if (pTag != NULL)
        {
            if(pTag->CAL_STATUS == _CAL_OK)
            {
                ...
                ECS_Param.BitSel = pFmt->header.Ecs_bit;
                ECS_Param.DthrEn = DISABLE;
                ECS_Param.DthrRst = DISABLE;
                ECS_Param.ECSDthrLv = 0;

                subs->Ecs.pecs = &ECS_Param;
                subs->Ecs.Inaddr = (UINT32)&pFmt->ECSBlockWin;
                subs->Ecs.EcsMap = pFmt->header.Ecs_map;
        ...
        {
            DBG_ERR("Cal Status Fail -> Load Default ECS GainTbl\r\n");
            #if 0
            *FuncEn &= ~ECSEn;
            *Update |= SIE_SubFEn;
            #else
